...
/*
if(E->parallel.me==0){
	if(element==1) printf("debug elt_K\n");
	if((element-1)%(E->lmesh.nox*E->lmesh.noy)==0){
		printf("elt_K of %d\n",element);
		int debug_i, debug_j;
		for(debug_i=0; debug_i<24; debug_i++){
			for(debug_j=0; debug_j<24; debug_j++)
				printf("%e\t",elt_K[debug_i*24+debug_j]);
			printf("\n");
		}
	}
}
*/
	    if (E->control.augmented_Lagr && !E->ve_data_cont.compressible)
...
int layers_r(struct All_variables *E,float r)
{
  ...
  */
  rl = r + E->sphere.ro;
  llayers = 0;
  if (r> (E->sphere.ro - E->viscosity.zbase_layer[E->viscosity.rheol_layers-1])){
    i = E->viscosity.rheol_layers;
    }
  else  {
    for(i=0;i<E->viscosity.rheol_layers-1;i++)
      if(r >= (E->sphere.ro - E->viscosity.zbase_layer[i] - 1e-6) && r<(E->sphere.ro - E->viscosity.zbase_layer[i+1]+1e-6)){
        i++;
        break;
       }
     }
...
int nlayers(struct All_variables *E,int kk)
{
  int llayers, i;
  llayers = 0;

  if (kk >= E->viscosity.nbase_layer[E->viscosity.rheol_layers-1])  {
     i = E->viscosity.rheol_layers;
     }
  else {
    for(i=0;i<E->viscosity.rheol_layers-1;i++)
      if (kk >= E->viscosity.nbase_layer[i]&&kk<E->viscosity.nbase_layer[i+1]){
      i++;
      break;
      }
    }
...
*/
 ...
 for (m=1;m<=E->sphere.caps_per_proc;m++)   {
   for (k=1;k<=E->lmesh.elz;k++)  {
     kk = k + E->lmesh.ezs; 
     llayer = nlayers(E,kk);
     for (j=1;j<=E->lmesh.ely;j++)
     for (i=1;i<=E->lmesh.elx;i++)   {
        el = k + (i-1)*E->lmesh.elz+(j-1)*E->lmesh.elx*E->lmesh.elz;
        E->mat[m][el] = llayer;
        }
      }
    }

  if (E->ve_data_cont.variable_te) {
     read_reg_grids(E,E->ve_data_cont.te_file,E->slice_ve.elast_thick);
     modify_mat_te(E,E->slice_ve.elast_thick);
     }

  if (E->ve_data_cont.plate_margins) {
     read_reg_grids(E,E->ve_data_cont.platemargins_file,E->slice_ve.plate_margins);
     }
...
void modify_mat_te(E,elast_thick)
     struct All_variables *E;
     double **elast_thick;
{
  int m,i,j,k,kk,el,e,a,nodea,els;
  double te_ele,center_r;
  const int dims=E->mesh.nsd,dofs=E->mesh.dof;
  const int ends=enodes[dims];

  for (m=1;m<=E->sphere.caps_per_proc;m++)   
  for (i=1;i<=E->lmesh.nsf;i++) {
            // convert elast_thick in km to radial coordinates
    elast_thick[m][i] = E->sphere.ro - elast_thick[m][i]*1000.0/E->sphere.dradius;
    }

  for (m=1;m<=E->sphere.caps_per_proc;m++)   
    for (j=1;j<=E->lmesh.ely;j++)  
    for (i=1;i<=E->lmesh.elx;i++)  {
      els = i + (j-1)*E->lmesh.elx;
      te_ele = 0.25*(elast_thick[m][E->sien[m][els].node[1]]
                    +elast_thick[m][E->sien[m][els].node[2]]
                    +elast_thick[m][E->sien[m][els].node[3]]
                    +elast_thick[m][E->sien[m][els].node[4]]);
      for (k=1;k<=E->lmesh.elz;k++)   {
        el = k + (i-1)*E->lmesh.elz + (j-1)*E->lmesh.elz*E->lmesh.elx;
        center_r = 0.0;
        for (a=1;a<=ends;a++) { 
           center_r += E->sx[m][3][E->ien[m][el].node[a]];
           }
        center_r = center_r/ends;    // radial loc of center of element

        E->mat[m][el] = 1;
        kk = k + E->lmesh.ezs; 
        if (center_r >= te_ele)  
 	   E->mat[m][el] = 2;   // elastic lithosphere
//        else if (kk>=E->viscosity.nlith1) 
//           E->mat[m][el] = 3;
        else if (kk>=E->viscosity.n410) 
           E->mat[m][el] = 4;
        else if (kk>=E->viscosity.nlm) 
           E->mat[m][el] = 5;
        else if (kk>=E->viscosity.nlm1) 
           E->mat[m][el] = 6;
      }
    }
