     ...
     int el,m,sphere_key;
{
    ...
    if (sphere_key)
        for(a=1;a<=ends;a++)   {
            node = E->ien[m][el].node[a];
/*            VV[1][a] = E->U[m][E->id[m][node].doff[1]];
            VV[2][a] = E->U[m][E->id[m][node].doff[2]];
            VV[3][a] = E->U[m][E->id[m][node].doff[3]]; */
            VV[1][a] = E->sphere.cap[m].V[1][node];
            VV[2][a] = E->sphere.cap[m].V[2][node];
            VV[3][a] = E->sphere.cap[m].V[3][node];  
        }
        ...
        for(a=1;a<=ends;a++)   {
            ...
            VV[1][a] = E->U[m][E->id[m][node].doff[1]]*cost*cosf
                - E->U[m][E->id[m][node].doff[2]]*sinf
                + E->U[m][E->id[m][node].doff[3]]*sint*cosf;
            VV[2][a] = E->U[m][E->id[m][node].doff[1]]*cost*sinf
                + E->U[m][E->id[m][node].doff[2]]*cosf
                + E->U[m][E->id[m][node].doff[3]]*sint*sinf;
            VV[3][a] = -E->U[m][E->id[m][node].doff[1]]*sint
                + E->U[m][E->id[m][node].doff[3]]*cost;
   ...
   for(e=1;e<=E->lmesh.NEL[lev];e++)   {
     for(j=1;j<=ends;j++)                {
       n = E->IEN[lev][m][e].node[j];
		//assert(n<=E->lmesh.NNO[lev]); // add by tao: debug E->ID
