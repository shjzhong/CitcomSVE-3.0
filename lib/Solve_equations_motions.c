...
static void solve_Ahat_fhat(struct All_variables *E,
                              double **V, double **F,
                              double imp, int *steps_max);
...

{
    ...
    if (!E->ve_data_cont.compressible)   {     // incompressible VE
       solve_Ahat_p_fhat(E,E->U,E->P,E->F,E->control.accuracy,&cycles);
       p_to_nodes(E,E->P,E->NP,E->mesh.levmax);
       }
    else if (E->ve_data_cont.compressible)   {  // compressible VE
       solve_Ahat_fhat(E,E->U,E->F,E->control.accuracy,&cycles);
       }
    v_from_vector(E);
...
/* Solve viscoelastic deformation of compressible mantle
 */
                                 ...
                                 double imp, int *steps_max)
{
    ...
    static double *F[NCS];  // add 'static' by tao
    ...
    time0 = CPU_time0();
    count = 0;
    v_res = sqrt(global_vdot(E,FF,FF,lev)/gneq);

    if (E->parallel.me==0) {
      fprintf(E->fp,"initial residue of momentum equation F %.9e %d %d\n",v_res,gneq,E->monitor.solution_cycles);
      fprintf(stderr,"initial residue of momentum equation F %.9e %d %d\n",v_res,gneq,E->monitor.solution_cycles);
      }
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)
        for(j=0;j<neq;j++)
            F[m][j] = FF[m][j];
    aimp = imp*v_res;
    valid = solve_del2_u(E, E->u1, F, aimp, lev);
    ...
    for(m=1; m<=E->sphere.caps_per_proc; m++)
      ...
      fflush(E->fp);
	if (E->parallel.me==0)
      fprintf(stderr,"Au=F solve after %g seconds for step %d\n",CPU_time0()-time0,E->monitor.solution_cycles);
                                 ...
                                 double imp, int *steps_max)
{
    ...
    time0 = CPU_time0();
    count = 0;
    v_res = sqrt(E->monitor.fdotf/gneq);
    v_res = sqrt(global_vdot(E,FF,FF,lev)/gneq);
    ...
    aimp = imp*v_res*E->control.sob_tolerance;    //one order higher accuracy
    if(E->control.inv_gruneisen == 0)
        initial_vel_residual(E, V, P, F, aimp);
...
/*        print_convergence_progress(E, count, time0,
    ...
    while( count==0 ||( (count < *steps_max) &&
           (dvelocity > imp || dpressure > imp))) {
//           (E->monitor.incompressibility > E->control.tole_comp) && (dvelocity > imp || dpressure > imp))) {
        ...
        if(count == 0)
            ...
            r0dotz0 = global_pdot(E,r0,z0,lev);
            assert(r0dotz0 != 0.0  /* Division by zero in head of incompressibility iteration */);
        ...
        aimp = imp*v_res*E->control.sob_tolerance;
        valid = solve_del2_u(E, E->u1, F, aimp, lev);
        if(!valid && (E->parallel.me==0)) {
            fputs("Warning: solver not converging! 1\n", stderr);
            fputs("Warning: solver not converging! 1\n", E->fp);
        ...
        E->monitor.incompressibility=sqrt((gneq/gnpno)*(1e-32+global_pdot(E,F,F,lev)/(1e-32+E->monitor.vdotv)));
        dvelocity = alpha * sqrt(global_vdot(E, E->u1,E->u1,lev)/(1e-32 + E->monitor.vdotv));
        dpressure = alpha * sqrt(global_pdot(E,s2,s2,lev)/(1e-32 + global_pdot(E,P,P,lev)));

        count++;


        if (E->control.print_convergence && E->parallel.me==0)  {
      fprintf(E->fp,"AhatP (%03d) after %g seconds with div/v=%.3e dv/v=%.3e dp/p=%.3e for step %d\n",count,CPU_time0()-time0,E->monitor.incompressibility,dvelocity,dpressure,E->monitor.solution_cycles);
      fflush(E->fp);
      fprintf(stderr,"AhatP (%03d) after %g seconds with div/v=%.3e dv/v=%.3e dp/p=%.3e for step %d\n",count,CPU_time0()-time0,E->monitor.incompressibility,dvelocity,dpressure,E->monitor.solution_cycles);
/*            print_convergence_progress(E, count, time0,
                                       v_norm, p_norm,
                                       dvelocity, dpressure,
                                       E->monitor.incompressibility); */
