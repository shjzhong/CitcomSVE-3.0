...
if(0){
if(E->parallel.me==0){
    printf("----- debug E->ID (just after construct_id called)------\n");
int m, i_debug, node_debug, eqn_debug,level_debug;
int ends=8;
for(m=1;m<=E->sphere.caps_per_proc;m++)
for(i_debug=1;i_debug<=ends;i_debug++){
    level_debug = E->mesh.gridmax;
    node_debug=E->IEN[level_debug][m][1].node[i_debug];
    eqn_debug = E->ID[level_debug][m][node_debug].doff[1];
    printf("node: %d,  eqn1: %d \n",node_debug,eqn_debug);
}
}
}
*/
      ...
      if(chatty)fprintf(stderr," mass matrix done\n");
		...
		if(E->parallel.me==0){
			printf("---debug E->TWW -------\n");
				...
				sprintf(debug_outfile, "debug_TWW_in_Instruction.%d",E->monitor.solution_cycles);
				FILE * debug_fp;
				debug_fp = fopen(debug_outfile, "w");
				int debug_e, debug_i;
				for(debug_e=1; debug_e<E->lmesh.nel; debug_e++){
					for(debug_i=1; debug_i<=enodes[E->mesh.nsd]; debug_i++)
						fprintf(debug_fp,"%e ", E->TWW[E->mesh.levmax][1][debug_e].node[debug_i]);
					fprintf(debug_fp,"\n");
				}
				fclose(debug_fp);
		}
...
void read_initial_settings(struct All_variables *E)
{
  ...
  */
  ...
  E->ve_data_cont.tau = E->data.ref_viscosity/(E->ve_data_cont.shear_mod*365.25*24*3600);

/*  if (E->ve_data_cont.Heaviside==1)  {   
        input_int("stages",&(E->ve_data_cont.stages),"0",m);
        input_int_vector("step",E->ve_data_cont.stages,(E->ve_data_cont.stages_step),m);
        input_double_vector("timestep",E->ve_data_cont.stages,(E->ve_data_cont.stages_time),m);
...
//if (E->parallel.me==0) fprintf(stderr,"bbb %d %d %g %g %d\n",i,j,temp0,temp1,temp2);
         }
       fclose(fp);
       temp2 = 0;
       for (i=0;i<E->ve_data_cont.stages;i++)  {
