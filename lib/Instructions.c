...
void elastic_viscosity_input(struct All_variables*);
...
void set_free_surfaces (struct All_variables *);
void set_mech_model_from_files (struct All_variables *);
...
void initial_mesh_solver_setup(struct All_variables *E)
{
  ...
  chatty = E->parallel.me == 0;
    ...
    /* construct_c3x3matrix(E); */
      ...
      if(chatty)fprintf(stderr," elt_cs done\n");
    mass_matrix(E);
    ...
    if(chatty)fprintf(stderr,"set_sphere_h done\n");

    set_mech_model_from_files(E);

    set_free_surfaces (E);
    if(chatty)fprintf(stderr,"set_free_surface done\n");
...
{
    void general_equations_of_motions_solver_setup();
    void initial_mesh_solver_setup();


    initial_mesh_solver_setup(E);

    general_equations_of_motions_solver_setup(E);
...
void read_initial_settings(struct All_variables *E)
{
  ...
  */
  ...
  input_string("te_datafile",E->ve_data_cont.te_file,"initialize",m);
  input_string("platemargins_datafile",E->ve_data_cont.platemargins_file,"initialize",m);
  input_string("ice_datafile",E->ve_data_cont.ice_file,"initialize",m);
  input_string("ocean_datafile",E->ve_data_cont.ocean_file,"initialize",m);

  E->ve_data_cont.read_ice_format=0;
  input_int("ice_sph_or_grid",&(E->ve_data_cont.read_ice_format),"0",m);

  E->ve_data_cont.variable_te=0;
  input_int("variable_te",&(E->ve_data_cont.variable_te),"off",m);
  E->ve_data_cont.Heaviside=0;
  input_int("Heaviside",&(E->ve_data_cont.Heaviside),"0",m);
  E->ve_data_cont.apply_potential=0;
  input_int("apply_potential",&(E->ve_data_cont.apply_potential),"0",m);
  E->ve_data_cont.plate_margins=0;
  input_int("platemargins",&(E->ve_data_cont.plate_margins),"off",m);
  
  input_int("update_K_steps",&(E->ve_data_cont.KERNEL),"0",m);
  input_int("iteration",&(E->ve_data_cont.iteration),"0",m);
  input_double("damping",&(E->ve_data_cont.damping),"1",m);
  input_double("tole_compressibility",&(E->control.tole_comp),"1e-7",m);

  input_int("self_gravitation",&(E->ve_data_cont.SELFG),"0",m);
  input_int("apply_SLE",&(E->ve_data_cont.SLE),"0",m);
  input_int("polar_wander",&(E->ve_data_cont.polar_wander),"0",m);
  input_double("polar_wander_kf",&(E->ve_data_cont.kf),"1",m);
  input_int("SLE_lmax",&(E->ve_data_cont.SLE_lmax),"0",m);
  input_int("pure_visc",&(E->ve_data_cont.pure_visc),"0",m);
  ...
  input_float("radius",&E->sphere.dradius,"6371e3.0",m);
  E->data.radius_km = E->sphere.dradius / 1e3;
  ...
  input_int("num_mat_layer",&(E->viscosity.rheol_layers),"1",m);   // number of rheology layers
  input_int("num_mat",&(E->viscosity.num_mat),"1",m);            // number of material groups. 
         // num_mat_layer <= num_mat. equal mostly, but smaller if weak plate margins are considered, 
  ...
  E->viscosity.nbase_layer[0] = 1;
  input_int_vector("n_mat_layer",E->viscosity.rheol_layers,(E->viscosity.nbase_layer),m);
  ...
  */
  ...
  input_int("storage_spacing_surface",&(E->control.record_every),"10",m);
  input_int("storage_vol",&(E->control.record_vol),"0",m);
  input_int("storage_spacing_vol",&(E->control.record_vol_every),"10",m);
  input_int("record_stress_tensor",&(E->control.record_stress_tensor),"0",m);
  ...
  input_int("max_mg_cycles",&(E->control.max_mg_cycles),"100,0,nomax",m);
  ...
  input_double("shearmodulus",&(E->ve_data_cont.shear_mod),"1e11",m);
  input_float("refvisc",&(E->data.ref_viscosity),"1.0e21",m);

  E->control.sob_tolerance = 1.0;
  input_float("additional_acc",&(E->control.sob_tolerance),"1.0",m);

  E->ve_data_cont.tau = E->data.ref_viscosity/(E->ve_data_cont.shear_mod);
  E->ve_data_cont.tau_in_years = E->data.ref_viscosity/(E->ve_data_cont.shear_mod*365.25*24*3600);

       input_int("NonZero_Init_ICELoad",&(E->ve_data_cont.NonZero_Init_ICELoad), "0",m);

       input_string("load_stages_time_file",E->ve_data_cont.timestage_file,"initialize",m);
       if ( (fp=fopen(E->ve_data_cont.timestage_file,"r"))==NULL) {
          fprintf(stderr,"ERROR: cannot find file %s\n",E->ve_data_cont.timestage_file);
          parallel_process_termination();
          }
       fgets(input_s,200,fp);
       sscanf(input_s,"%d %d",&E->ve_data_cont.stages,&j);
// j=0: in dimensionless maxwell time;   j=1: k years, 
       for (i=0;i<=E->ve_data_cont.stages;i++)  {
         fgets(input_s,200,fp);
         sscanf(input_s,"%lf %d",&temp1,&temp2);
         if (j==1) temp1 = temp1*1000.0;
         if (i==0)temp0=-temp1;
         E->ve_data_cont.stages_time[i]=temp1+temp0;
         E->ve_data_cont.stages_step[i]=temp2;
         }
       fclose(fp);

       temp2 = 0;
       for (i=0;i<E->ve_data_cont.stages;i++)  {
          E->ve_data_cont.stages_time[i]=E->ve_data_cont.stages_time[i+1]-E->ve_data_cont.stages_time[i];
          if(i>0) E->ve_data_cont.stages_step[i]+=temp2;
          temp2 =E->ve_data_cont.stages_step[i];
          }
       E->advection.max_timesteps = temp2;
       E->advection.max_total_timesteps = temp2;

    oldstep = 0;
    for (i=0;i<E->ve_data_cont.stages;i++)  {
//        if (E->ve_data_cont.Heaviside==1 || E->ve_data_cont.Heaviside==3)
        if (j==0)      // i.e., dimensionless time
            E->ve_data_cont.stages_timestep[i] = E->ve_data_cont.stages_time[i]
                /((E->ve_data_cont.stages_step[i]-oldstep));
        else       // i.e., originally in k years or years, turn to dimensionless
            E->ve_data_cont.stages_timestep[i] = E->ve_data_cont.stages_time[i]
                /(E->ve_data_cont.tau_in_years*(E->ve_data_cont.stages_step[i]-oldstep));
...
if (E->parallel.me==0) fprintf(E->fp,"neqneq %d %d\n",E->mesh.neq, E->mesh.nnov);
  ...
  E->sphere.snel = E->sphere.elx*E->sphere.ely;
  E->sphere.nsf = E->sphere.nox*E->sphere.noy;
...

{
 ...
 for (j=1;j<=E->sphere.caps_per_proc;j++)  {
  ...
  for(i=1;i<=E->mesh.nsd;i++)
   ...
   E->gDA0[j] = (struct Shape_function_dA *)malloc((snel+1)*sizeof(struct Shape_function_dA));
   E->gDA1[j] = (struct Shape_function_dA *)malloc((snel+1)*sizeof(struct Shape_function_dA));
  ...
  E->slice_ve.all_load[j]      = (double *)malloc((nno+2)*sizeof(double));
  E->slice_ve.all_stress[j]     = (double *)malloc((nno+2)*sizeof(double));
  E->slice_ve.total_load[j]      = (double *)malloc((nno+2)*sizeof(double));
  E->slice_ve.all_potential[j]      = (double *)malloc((nno+2)*sizeof(double));
  E->slice_ve.all_load_potential[j]      = (double *)malloc((nno+2)*sizeof(double));
  E->slice_ve.total_potential[j]      = (double *)malloc((nno+2)*sizeof(double));
  E->slice_ve.init_total_potential[j] = (double *)malloc((nno+2)*sizeof(double));

  E->potential[0][j]      = (double *)malloc((nsf+2)*sizeof(double));
  E->potential[1][j]      = (double *)malloc((nsf+2)*sizeof(double));
  E->init_potential[0][j]      = (double *)malloc((nsf+2)*sizeof(double));
  E->init_potential[1][j]      = (double *)malloc((nsf+2)*sizeof(double));
  E->incr_potential[0][j]      = (double *)malloc((nsf+2)*sizeof(double));
  E->incr_potential[1][j]      = (double *)malloc((nsf+2)*sizeof(double));
  E->incr_potential[2][j]      = (double *)malloc((nsf+2)*sizeof(double));
  E->incr_potential[3][j]      = (double *)malloc((nsf+2)*sizeof(double));
  E->slice_ve.elast_thick[j]=(double *)malloc((nsf+2)*sizeof(double));
  E->slice_ve.plate_margins[j]=(double *)malloc((nsf+2)*sizeof(double));

  for (i=1;i<=nno;i++) 		{
     E->slice_ve.all_load[j][i] = 0.0;
     E->slice_ve.total_load[j][i] = 0.0;
     E->slice_ve.total_potential[j][i] = 0.0;
     E->slice_ve.all_potential[j][i] = 0.0;
     E->slice_ve.all_load_potential[j][i] = 0.0;
     E->slice_ve.init_total_potential[j][i] = 0.0;
     }


  for (i=0;i<=5;i++)   {
     E->slice_ve.load[i][j]      = (double *)malloc((nsf+2)*sizeof(double));
     for (k=0;k<=nsf;k++)
       E->slice_ve.load[i][j][k] = 0.0;
   }
  for (i=1;i<=E->mesh.nsd;i++)   {
     E->slice_ve.surf[i][j]      = (double *)malloc((nsf+2)*sizeof(double));
     E->slice_ve.botm[i][j]      = (double *)malloc((nsf+2)*sizeof(double));
     E->Xsurf[i][j]      = (double *)malloc((nsf+2)*sizeof(double));
     for (k=0;k<=nsf;k++)   {
       E->slice_ve.surf[i][j][k] = 0.0;
       E->slice_ve.botm[i][j][k] = 0.0;
     }
   }
 // for ocean loading with SLE
   E->slice_ve.dynamic_oceanload[j]=(double *)malloc((nsf+2)*sizeof(double));
   E->slice_ve.static_oceanload[j]=(double *)malloc((nsf+2)*sizeof(double));
   E->slice_ve.iceload[0][j]=(double *)malloc((nsf+2)*sizeof(double));
     for (k=0;k<=nsf;k++)   {
       E->slice_ve.dynamic_oceanload[j][k] = 0.0;
       E->slice_ve.static_oceanload[j][k] = 0.0;
       E->slice_ve.iceload[0][j][k] = 0.0;
     }

  E->Maxwelltime[j] = (double *)malloc((nel+1)*sizeof(double));
  E->Sxx[j] = (double *)malloc((nel*vpts+1)*sizeof(double));
  E->Syy[j] = (double *)malloc((nel*vpts+1)*sizeof(double));
  E->Szz[j] = (double *)malloc((nel*vpts+1)*sizeof(double));
  E->Sxy[j] = (double *)malloc((nel*vpts+1)*sizeof(double));
  E->Sxz[j] = (double *)malloc((nel*vpts+1)*sizeof(double));
  E->Szy[j] = (double *)malloc((nel*vpts+1)*sizeof(double));
...
//  	}
    ...
    E->StrainTensorXY[j] = (float *)malloc((nel+1)*sizeof(float));
    E->StrainTensorXZ[j] = (float *)malloc((nel+1)*sizeof(float));
    E->StrainTensorZY[j] = (float *)malloc((nel+1)*sizeof(float));
    E->StrainTensorNodeXX[j] = (float *)malloc((nno+1)*sizeof(float));
    E->StrainTensorNodeYY[j] = (float *)malloc((nno+1)*sizeof(float));
    E->StrainTensorNodeZZ[j] = (float *)malloc((nno+1)*sizeof(float));
    E->StrainTensorNodeXY[j] = (float *)malloc((nno+1)*sizeof(float));
    E->StrainTensorNodeXZ[j] = (float *)malloc((nno+1)*sizeof(float));
    E->StrainTensorNodeZY[j] = (float *)malloc((nno+1)*sizeof(float));
// for stress-dependent viscosity
    E->S2xx[j] = (double *)malloc((nel+1)*sizeof(double));
    E->S2yy[j] = (double *)malloc((nel+1)*sizeof(double));
    E->S2zz[j] = (double *)malloc((nel+1)*sizeof(double));
    E->S2xy[j] = (double *)malloc((nel+1)*sizeof(double));
    E->S2xz[j] = (double *)malloc((nel+1)*sizeof(double));
    E->S2zy[j] = (double *)malloc((nel+1)*sizeof(double));
    E->EVold[j] = (float *) malloc((nel+2)*sizeof(float));
    E->EVolder[j] = (float *) malloc((nel+2)*sizeof(float));
 ...
 for(i=E->mesh.gridmin;i<=E->mesh.gridmax;i++)
  for (j=1;j<=E->sphere.caps_per_proc;j++)  {
    ...
    E->erho[i][j]    = (float *) malloc((nel+1)*sizeof(float));
    E->elambda[i][j] = (float *) malloc((nel+1)*sizeof(float));
    E->esmu[i][j]    = (float *) malloc((nel+1)*sizeof(float));
    E->elambda_o[i][j] = (float *) malloc((nel+1)*sizeof(float));
    E->esmu_o[i][j]    = (float *) malloc((nel+1)*sizeof(float));
    E->evi_o[i][j]    = (float *) malloc((nel+1)*sizeof(float));
    E->egrav[i][j]    = (float *) malloc((nel+1)*sizeof(float));
    E->BPI[i][j] = (double *) malloc((npno+1)*sizeof(double));

	E->grav[i][j] = (float *) malloc((nno+1)*sizeof(float)); // add by tao: see defination; 
...

{
 ...
 n=1;
  for (j=1;j<=E->sphere.caps_per_proc;j++)   {
    ...
    for(i=1;i<=E->mesh.nsd;i++) {
      E->sphere.cap[j].V[i] = (double *) malloc((E->lmesh.nnov+1)*sizeof(double));
      E->sphere.cap[j].total_VS[i] = (double *) malloc((E->lmesh.nsf+1)*sizeof(double));
	  E->sphere.cap[j].total_VC[i] = (double *) malloc((E->lmesh.nsf+1)*sizeof(double));
      E->sphere.cap[j].total_V[i] = (double *) malloc((E->lmesh.nnov+1)*sizeof(double));  //add by tao: total_VS used to calculate Love Number.
    ...
    for(k=1;k<=E->mesh.nsd;k++)
      for(i=1;i<=E->lmesh.nnov;i++)  {
		...
		E->sphere.cap[j].total_V[k][i] = 0.0;
        }
    for(k=1;k<=E->mesh.nsd;k++)
      for(i=1;i<=E->lmesh.nsf;i++)  {   
        E->sphere.cap[j].total_VS[k][i] = 0.0;
        E->sphere.cap[j].total_VC[k][i] = 0.0;
        }
  ...
  for(l=E->mesh.gridmin;l<=E->mesh.gridmax;l++)
    for (j=1;j<=E->sphere.caps_per_proc;j++)   {
      ...
      for (i=1;i<=enodes[E->mesh.nsd];i++)  {
         E->B_R[l][j][i] = (double *)malloc((E->lmesh.SNEL[l]*onedvpoints[E->mesh.nsd]+2)*sizeof(double));
         for (n=1;n<=E->lmesh.SNEL[l]*onedvpoints[E->mesh.nsd]+1;n++)
             E->B_R[l][j][i][n] = 0.0;

		 E->B_RR[l][j][i] = (double *) malloc((E->lmesh.NEL[l]*onedvpoints[E->mesh.nsd]+2)*sizeof(double));
	  	 for (n=1;n<=E->lmesh.NEL[l]*onedvpoints[E->mesh.nsd]+1;n++)
		     E->B_RR[l][j][i][n]=0.0;
...
static void open_log(struct All_variables *E)
{
  ...
  sprintf(logfile,"%s.Love_numbers", E->control.data_file);
  E->fp_LN = output_open(logfile, "w");
