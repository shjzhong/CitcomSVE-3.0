     ...
     struct All_variables *E;
{
  ...
  for(lv=E->mesh.gridmax;lv>=E->mesh.gridmin;lv--)
    for (j=1;j<=E->sphere.caps_per_proc;j++)     {
      noz = E->mesh.NOZ[lv];
      if(E->mesh.topvbc != 1) {	/* free stress top, note: free-slip BC is removed */
	...
	horizontal_bc(E,E->sphere.cap[j].VB,noz,3,E->control.VBZtopval,SBZ,1,lv,j);
	horizontal_bc(E,E->sphere.cap[j].VB,noz,2,E->control.VBYtopval,SBY,1,lv,j);
	}
      if(E->mesh.botvbc != 1) {	/* free stress bottom */
        ...
        horizontal_bc(E,E->sphere.cap[j].VB,1,3,E->control.VBZbotval,SBZ,1,lv,j);
