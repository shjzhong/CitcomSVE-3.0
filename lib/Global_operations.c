...
//        E->U[m][E->id[m][node].doff[2]] = E->sphere.cap[m].V[2][node];
...

    {
      ...
      E->ve_data_cont.Omega_surface[0] *= 180.0/M_PI / E->ve_data_cont.tau;  // convert to degrees per year, seems need (E->ve_data_cont.tau*E->advection.timestep)
      E->ve_data_cont.Omega_surface[1] *= 180.0/M_PI / E->ve_data_cont.tau;  
      E->ve_data_cont.Omega_surface[2] *= 180.0/M_PI / E->ve_data_cont.tau;  
      // written to topo_s
...
// icon all 0 when called.
void load_to_CM_grav_comp(struct All_variables *E, int load_only, int icon, int add_incr_CM)
{
...
if(E->parallel.me==0)
	...
	calculate_potential_comp_OR_deg1_2(E, E->slice_ve.all_potential, load_only, icon, 1, add_incr_CM);
...
//		   E->sphere.cap[m].total_V[1][i] += u1;
//	       E->sphere.cap[m].total_V[2][i] += u2;
//	       E->sphere.cap[m].total_V[3][i] += u3;
//		}
//	else
		{//comment by tao: total_VS used for Love Number calculation. 
	    if ( k==E->lmesh.noz && E->parallel.me_loc[3]==E->parallel.nprocz-1) {
