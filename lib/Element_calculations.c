...
//
  ...
  for (i=0;i<24;i++)
	...
	if(E->parallel.me==0 && E->advection.timesteps%50==0){
		time = CPU_time0() ;
		fprintf(E->fp,"DEBUG (In assemble_force) : begin , time = %f \n",time);
	}
...
// Set slice.surf/botm[2] for use in get_potential:
   get_boundary_forces(E,count);
	if(E->parallel.me==0 && E->advection.timesteps%50==0){
                time = CPU_time0() -time;
                fprintf(E->fp,"DEBUG (In assemble_force) : after get_boundary_forces , delta time = %f \n",time);
		time = CPU_time0();
        }
	/*	
		if(1||E->parallel.me==0 || E->parallel.me==1){
			...
			sprintf(debug_outfile, "debug_all_stress_Mine.%d.%d.proc%d",E->monitor.solution_cycles,count, E->parallel.me);
			FILE * debug_fp;
	...
	*/
	/*	
		float global_tdot_d();
		float temp =0;
		temp =  global_tdot_d(E,E->slice_ve.all_stress,E->slice_ve.all_stress, lev);
...
/*
    ...
    printf("all_stress(1 in assemble force): %e; from core %d ; count: %d\n", _temp1, E->parallel.me, E->monitor.count);	
*/
	/*
		if(E->parallel.me==0){
			printf("---debug E->U (before calc potential) (count = %d), neq=%d -----\n", count,neq);
			char debug_outfile[255];
			sprintf(debug_outfile, "debug_U_BeforePotential_Mine.%d.%d",E->monitor.solution_cycles,count);
			FILE * debug_fp;
			debug_fp = fopen(debug_outfile, "w");
			int debug_i;
			for(debug_i=0; debug_i<neq; debug_i++){

				if(debug_i%3==0) fprintf(debug_fp,"\n");
				fprintf(debug_fp,"%e  ", E->U[1][debug_i]);
			}
			fclose(debug_fp);
		}
		*/
   ...
   if (E->ve_data_cont.SELFG)
      get_potential(E,count);
        ...
        if(E->parallel.me==0 && E->advection.timesteps%50==0){
                time = CPU_time0() -time;
                fprintf(E->fp,"DEBUG (In assemble_force) : after get_potential, delta time = %f \n",time);
		time = CPU_time0();
        }

/*
		if(1 || E->parallel.me==0){
			printf("---debug E->init_total_potential (count = %d) -----\n", E->monitor.count);
			char debug_outfile[255];
			sprintf(debug_outfile, "debug_init_total_potential_Mine.%d.%d.%d",E->monitor.solution_cycles,E->monitor.count,E->parallel.me);
			FILE * debug_fp;
			debug_fp = fopen(debug_outfile, "w");
			int debug_i;
			for(debug_i=1; debug_i<=E->lmesh.nno; debug_i++){
...
//parallel_process_termination();
		...
		//if(E->parallel.me==0)
		/*{
			printf("---debug E->all_potential (count = %d) -----\n", count);
			char debug_outfile[255];
			sprintf(debug_outfile, "debug_all_potential_Mine.%d.%d.proc%d",E->monitor.solution_cycles,count,E->parallel.me);
			FILE * debug_fp;
			debug_fp = fopen(debug_outfile, "w");
			int debug_i;
			for(debug_i=1; debug_i<=E->lmesh.nno; debug_i++){
				fprintf(debug_fp,"%e\n", E->slice_ve.all_potential[1][debug_i]);
			}
			fclose(debug_fp);
		}
		*/
	...
	//	if(E->parallel.me==0)
		/*{
			printf("---debug E->total_potential (count = %d) -----\n", count);
			char debug_outfile[255];
			sprintf(debug_outfile, "debug_total_potential_Mine.%d.%d.proc%d",E->monitor.solution_cycles,count,E->parallel.me);
			FILE * debug_fp;
			debug_fp = fopen(debug_outfile, "w");
			int debug_i;
			for(debug_i=1; debug_i<=E->lmesh.nno; debug_i++){
				fprintf(debug_fp,"%e\n", E->slice_ve.total_potential[1][debug_i]);
			}
			fclose(debug_fp);
		}
		*/
...
/*
    ...
    double _temp;
    _temp =  global_tdot_d(E,E->slice_ve.total_potential,E->slice_ve.total_potential, E->mesh.levmax);
    printf("total_potential (in assemble force): %e; from core %d \n", _temp, E->parallel.me);
*/
/*
        double global_tdot_d();
        double temp;
        temp =  global_tdot_d(E,E->slice_ve.init_total_potential,E->slice_ve.init_total_potential, lev);
        printf(" init_total_potential: %e; from core %d \n", temp, E->parallel.me);
        temp =  global_tdot_d(E,E->slice_ve.all_potential,E->slice_ve.all_potential, lev);
        printf(" all_potential       : %e; from core %d \n", temp, E->parallel.me);
        temp =  global_tdot_d(E,E->slice_ve.total_potential,E->slice_ve.total_potential, lev);
        printf(" total_potential: %e; from core %d \n", temp, E->parallel.me);
*/
	...
	if (E->ve_data_cont.SLE)
        ...
        if(E->parallel.me==0 && E->advection.timesteps%50==0){
                time = CPU_time0() -time;
                fprintf(E->fp,"DEBUG (In assemble_force) : after get_dynamic_oceanload , delta time = %f \n",time);
		time = CPU_time0();
        }
    ...
    if(E->ve_data_cont.compressible){
        ...
        if(E->parallel.me==0 && E->advection.timesteps%50==0){
                time = CPU_time0() -time;
                fprintf(E->fp,"DEBUG (In assemble_force) : after append_multi_boundary_forces , delta time = %f \n",time);
                time = CPU_time0();
        }

	//<<<< end tao
/*
    _temp1 =  global_tdot_d_noskip(E,E->slice_ve.all_load,E->slice_ve.all_load, E->mesh.levmax);
    printf("all_load(1 in assemble force): %e; from core %d ; count: %d\n", _temp1, E->parallel.me, E->monitor.count);

    _temp1 =  global_tdot_d_noskip(E,E->slice_ve.total_load,E->slice_ve.total_load, E->mesh.levmax);
    printf("total_load(1 in assemble force): %e; from core %d ; count: %d\n", _temp1, E->parallel.me, E->monitor.count);
*/
//parallel_process_termination();
	// collect pre-stresses in F0
    //
		
		// if(E->parallel.me==0)
		/*{
			printf("---debug E->all_load (count = %d) -----\n", count);
			char debug_outfile[255];
			sprintf(debug_outfile, "debug_all_load_Mine.%d.%d.%d",E->monitor.solution_cycles,count,E->parallel.me);
			FILE * debug_fp;
			debug_fp = fopen(debug_outfile, "w");
			int debug_i;
			for(debug_i=1; debug_i<=E->lmesh.nno; debug_i++){
				fprintf(debug_fp,"%e\n", E->slice_ve.all_load[1][debug_i]);
			}
			fclose(debug_fp);
		}*/
		...
		// if(E->parallel.me==0)
	/*	{
            printf("---debug E->total_load (count = %d) -----\n", count);
            char debug_outfile[255];
            sprintf(debug_outfile, "debug_total_load_Mine.%d.%d.%d",E->monitor.solution_cycles,count,E->parallel.me);
            FILE * debug_fp;
            debug_fp = fopen(debug_outfile, "w");
            int debug_i;
            for(debug_i=1; debug_i<=E->lmesh.nno; debug_i++){
                fprintf(debug_fp,"%e\n", E->slice_ve.total_load[1][debug_i]);
            }
            fclose(debug_fp);
        }
	*/
...
//            get_elt_f(E,e,elt_f,1,m);
            ...
            for(a=1;a<=ends;a++)  {
				...
				/*if(E->monitor.solution_cycles==0){
					assert(abs(F0[m][a1])<1e-30);
					assert(abs(F0[m][a2])<1e-30);
					assert(abs(F0[m][a3])<1e-30);
				}*/
                }
             }
        ...
        if(E->parallel.me==0 && E->advection.timesteps%50==0){
                time = CPU_time0() -time;
                fprintf(E->fp,"DEBUG (In assemble_force) : after get_elt_ps , delta time = %f \n",time);
                time = CPU_time0();
        }
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++)    {
    ...
    for (e=1;e<=nel;e++)  {
		   ...
		   if(E->ve_data_cont.compressible){
				...
				/* 
				if(E->parallel.me==0){
					if(e==0) printf("debug last 3 forces term\n");
					if((int)((e-1)/(E->lmesh.elx*E->lmesh.ely))==15){
						printf("elt_f of element %d \n",e);
						int debug_i, debug_j;
						for(debug_i=1; debug_i<=ends; debug_i++){
							node = E->ien[m][e].node[debug_i];
							p=(debug_i-1)*dims;
							printf("\t node %d force: %e %e %e \n", debug_i, elt_f[p], elt_f[p+1], elt_f[p+2]);
						}
					}	
				}
				*/
        ...
        if(E->parallel.me==0 && E->advection.timesteps%50==0){
                time = CPU_time0() -time;
                fprintf(E->fp,"DEBUG (In assemble_force) : after load_boundary_forces , delta time = %f \n",time);
        }
