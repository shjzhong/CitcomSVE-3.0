...
#define FBZ 0x100000
#define FBY 0x200000
#define BOUNDARIES 0x10
#define BOUNDARY1 0x1
#define BOUNDARY2 0x2
#define RESTORE 0x800000
...
struct SPHERE   {
  ...
  float *harm_tpgb[2];
  float dradius;
  higher_precision *sx[4];
  double *con;
  higher_precision *det[5];

  double **tablesplm[NCS];
  double **tablescosf[NCS];
  double **tablessinf[NCS];
  double **tablesplm_n[NCS];
  double **tablescosf_n[NCS];
  double **tablessinf_n[NCS];
  ...
  double *sphc[10],*sphs[10]; 
  double *sphc_init_load,*sphs_init_load;  // add by tao: for initial load potential.
  double *sphc_c[300], *sphs_c[300]; // add by tao, phi_c, increase the r dimension to 300.
  double *sphc_v[300], *sphs_v[300]; // add by tao, phi_v, calculate_potential_comp
  int nox;
  int noy;
  int noz;
  int nsf;
...
struct MONITOR {
	...
	int count; // add by tao: for force iteration count.
    int stop_topo_loop;
    int topo_loop;
...
#define NCS      14
	...
	struct VE_DATA_CONT {
		...
		double	 tau;	// maxwell time in yrs (t*tau = dimensional time)
		double	 Te;
	   ...
	   // the following are for CM (center of mass) calculations:
		...
		int LAMBDA_FROM_FILE; // option for read 3D lambda
		int MU_FROM_FILE; // option for read 3D mu
		char lambda_file[100];
		char mu_file[100];
	  } ve_data_cont;
	...
	double *Ekk[NCS]; /* add by tao: copy from geruo: cumulative e_(kk) term */
    ...
    struct SPH1 *Sph_Harm_Tblcs[NCS][101];
    struct SPH2 *Sph_Harm_Tbllm[NCS][5151];
    float *Tbl_cs[NCS][101],*Tbl_sn[NCS][101],*Tbl_lm[NCS][5151];
  ...
  //float *stress[NCS];
    ...
    float *EVold[NCS],*EVolder[NCS],*S2inv[NCS],*E2inv[NCS];
    float *erho[MAX_LEVELS][NCS],*elambda[MAX_LEVELS][NCS],*esmu[MAX_LEVELS][NCS],*egrav[MAX_LEVELS][NCS];
	float *grav[MAX_LEVELS][NCS];  // add by tao: egrav ele_to_nodes -> grav, used in force term.; 
	// comment by tao: geruo use a subroutine compute_g to get it and egrav
    ...
    int *mat[NCS];
    float *VIP[NCS];
    unsigned int *NODE[MAX_LEVELS][NCS],*ELEMENT[MAX_LEVELS][NCS];
    unsigned int *node[NCS],*element[NCS];

    float *age_t;

// related to the VE loading
    float *Te[NCS];
    double *Maxwelltime0[NCS];
    double *Maxwelltime[NCS],*Xsurf[4][NCS],*B_R[MAX_LEVELS][NCS][9],*stress[NCS];
    double *B_RR[MAX_LEVELS][NCS][9];
    double *Sxx[NCS],*Syy[NCS],*Szz[NCS],*Sxy[NCS],*Sxz[NCS],*Szy[NCS];
    double *S2xx[NCS],*S2yy[NCS],*S2zz[NCS],*S2xy[NCS],*S2xz[NCS],*S2zy[NCS];
    float *StrainTensorXX[NCS], *StrainTensorYY[NCS], *StrainTensorZZ[NCS], 
            *StrainTensorXY[NCS], *StrainTensorXZ[NCS], *StrainTensorZY[NCS];
    float *StrainTensorNodeXX[NCS], *StrainTensorNodeYY[NCS], *StrainTensorNodeZZ[NCS], 
            *StrainTensorNodeXY[NCS], *StrainTensorNodeXZ[NCS], *StrainTensorNodeZY[NCS];
      // allocated in allocate_common_vars(); implemented in update_stress_strain();
      // index from 1 to nel, not 0 to nel-1
      // use float instead of double b.c. of projection
    double *init_potential[4][NCS],*potential[4][NCS],*incr_potential[4][NCS],*pred_potential[4][NCS];
    ...
    struct Shape_function_dA *gDA0[NCS];
    struct Shape_function_dA *gDA1[NCS];
