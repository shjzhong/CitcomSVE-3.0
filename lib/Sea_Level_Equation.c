...
if (E->parallel.me_loc[3] == E->parallel.nprocz-1) {
    ...
    for (i=0;i<2;i++) {
        ...
        fprintf(fp,"Dynamic oceanload calculation at %.1f yrs: ",
                    E->ve_data_cont.tau*E->monitor.elapsed_time );    // time
    ...
    struct All_variables *E;
{
    ...
    if (!been_here) {   // first visit
        ...
        total_time = E->ve_data_cont.tau*E->monitor.elapsed_time;
    ...
    else  steps_per_epoch =  1000.0/E->ve_data_cont.tau   // nondim 1kyr
    ...
    struct All_variables *E;
{
   ...
   ifile = E->ve_data_cont.stage + 1;
    ...
    if (ifile==1)   {   // if this is not the first visit, curr->prev
       sprintf(outfile,"%s%dx%d.%d", 
           E->ve_data_cont.ocean_file, E->sphere.elx,E->sphere.ely, ifile-1);
   ...
   sprintf(outfile,"%s%dx%d.%d", 
            E->ve_data_cont.ocean_file, E->sphere.elx,E->sphere.ely, ifile);
