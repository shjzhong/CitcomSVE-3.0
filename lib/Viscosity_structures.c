...
void viscosity_system_input(struct All_variables *E)
{
	...
	E->ve_data_cont.LAMBDA_FROM_FILE = 0;
	input_int("3d_lambda_from_file", &(E->ve_data_cont.LAMBDA_FROM_FILE), "0",m);
	if (E->ve_data_cont.LAMBDA_FROM_FILE){
		input_string("3d_lambda_datafile", E->ve_data_cont.lambda_file, "initialize",m);
	}

	E->ve_data_cont.MU_FROM_FILE = 0;
	input_int("3d_mu_from_file", &(E->ve_data_cont.MU_FROM_FILE), "0",m);
	if (E->ve_data_cont.MU_FROM_FILE){
		input_string("3d_mu_datafile", E->ve_data_cont.mu_file, "initialize",m);
	}
	

    input_int("compressible",&(E->ve_data_cont.compressible),"0",m);

    E->ve_data_cont.OneDmodel_read=0;
     ...
     float **evisc,**visc;
{
    ...
    void get_shearmodulus();

    void mech_model_from_file();
	void model_from_file_3D();
...
/*
if(E->parallel.me==0){
    printf("----- debug E->ID (in get_system_viscosity; before read 1D model) ------\n");
int m, i_debug, node_debug, eqn_debug,level_debug;
int ends=8;
for(m=1;m<=E->sphere.caps_per_proc;m++)
for(i_debug=1;i_debug<=ends;i_debug++){
    level_debug = E->mesh.gridmax-3;
...
*/
    if (!E->ve_data_cont.OneDmodel_read) {   // use mat group
       get_shearmodulus(E);
       visc_from_mat(E,evisc);
    }
    else {              // read from a file and interpolate to FE grid
      // printf("read from %s \n", E->ve_data_cont.OneDmodel_file);
		mech_model_from_file(E,evisc);
	
       if (E->viscosity.FROM_FILE)   // read 3D viscosity
          visc_from_file(E,evisc,visc);

	   if (E->ve_data_cont.LAMBDA_FROM_FILE)
	   	  model_from_file_3D(E, E->elambda[E->mesh.levmax], E->ve_data_cont.lambda_file, E->ve_data_cont.shear_mod);

	   if (E->ve_data_cont.MU_FROM_FILE)
	   	  model_from_file_3D(E, E->esmu[E->mesh.levmax], E->ve_data_cont.mu_file, E->ve_data_cont.shear_mod);
    }

	// if(E->sphere.capid[1] == 1 && E->parallel.me_loc[1]==0 && E->parallel.me_loc[2]==0){
    // output elemental lambda mu
  if(0){              // (E->monitor.solution_cycles==1){
    FILE *fp_lambda, *fp_mu;
    char fp_lambda_name[100], fp_mu_name[100];
		int element;
    int indexz, indexy, indexx;
		int m=1;
    double t,f, r;
    double x,y,z;
    for(indexz=1; indexz<=E->lmesh.elz; indexz++){
      sprintf(fp_lambda_name, "elemental_lambda.dat_layer_%d.%d", indexz, E->parallel.me);
      sprintf(fp_mu_name, "elemental_mu.dat_layer_%d.%d", indexz, E->parallel.me);
      fp_lambda = fopen(fp_lambda_name, "w");
      fp_mu = fopen(fp_mu_name, "w");
  		for(indexx=1; indexx<=E->lmesh.elx; indexx++){
	  		for(indexy=1; indexy<=E->lmesh.ely; indexy++){
          element = indexz + (indexy-1)*E->lmesh.elz + (indexx-1)*E->lmesh.elz*E->lmesh.ely;
          t = E->eco[m][element].centre[1];
          f = E->eco[m][element].centre[2];
          r = E->eco[m][element].centre[3];
          // x = r*sin(t)*cos(f); y=r*sin(t)*sin(f); z=r*cos(t);
          fprintf(fp_lambda, "%e %e %e \n", t,f, E->elambda[E->mesh.levmax][m][element]);
          fprintf(fp_mu, "%e %e %e \n", t,f, E->esmu[E->mesh.levmax][m][element]);
        }
      }
      fclose(fp_lambda);
      fclose(fp_mu);
		}
	} // end of if(E->monitor.solution_cycles==1)


	if(E->sphere.capid[1] == 1 && E->parallel.me_loc[1]==0 && E->parallel.me_loc[2]==0){
		fprintf(E->fp,"debug earth model: node g, from proc %d\n", E->parallel.me);
		int node;
		int m=1;
		for(node=1; node<=E->lmesh.noz; node++){

			fprintf(E->fp, "%e %e %e ", E->sx[m][3][node],
										E->grav[E->mesh.levmax][m][node],
										E->grav[E->mesh.levmax][m][node] * 9.8 );
			fprintf(E->fp, "\n");
		}
	}
...
/*
if(E->parallel.me==0){
    printf("----- debug E->ID (in get_system_viscosity; before add_viscoelasticity_comp) ------\n");
int m, i_debug, node_debug, eqn_debug,level_debug;
int ends=8;
for(m=1;m<=E->sphere.caps_per_proc;m++)
for(i_debug=1;i_debug<=ends;i_debug++){
     ...
     struct All_variables *E;
{
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++)
    for(i=1;i<=E->lmesh.nel;i++)  {   
      E->Maxwelltime[m][i] = E->viscosity.G[E->mat[m][i]-1];
      }

  return;
 }

void mech_model_from_file(struct All_variables *E,float ** EEta)
//     struct All_variables *E;
//     float **EEta;
{
...
//	con = 3.1415926536*6.673e-11;
    ...
    for (i=1;i<=ndepth;i++)  {
      fgets(input_s,200,fp1);
      sscanf(input_s,"%g %g %g %g %g %g",&tmp1,&rho,&vp,&vs,&g,&visc);
      radius[i] = tmp1/E->sphere.dradius;
      model[1][i] = rho;
      model[2][i] = rho*vp*vp - 2.0*rho*vs*vs;
      model[3][i] = /*2.0* */ rho*vs*vs; // modified by tao: remove 2.0
      model[4][i] = g;
      model[5][i] = visc;
		//		if(E->parallel.me==0)
		//			 printf("%e %e %e %e %e %e \n", radius[i], model[1][i], model[2][i], model[3][i], model[4][i], model[5][i]);
      }
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++)
    for (jj=1;jj<=E->lmesh.elz;jj++)  {
	  ...
	  // compute_g: begin
/*	  if(E->parallel.me==0 && jj==1)	
	  	fprintf(stderr, "compute g from density. Not working for >1 cores on r direction\n");

	  rho2 = tmp1;
	  // g1 is g at base of the element. g2 is the g at rr.

	  if(jj==1){
		  r1 = E->sphere.ri*E->sphere.dradius; //E->sx[m][3][jj]
          	  g1 = 4./3.*con*E->data.density_below*r1;
	  	  r2 = rr*E->sphere.dradius;
		  g2 = g1*r1*r1/r2/r2 + 4./3.*con*rho2*(r2-pow(r1,3.)/r2/r2);
		  r2 = E->sx[m][3][jj+1]*E->sphere.dradius;
		  g1 = g1*r1*r1/r2/r2 + 4./3.*con*rho2*(r2-pow(r1,3.)/r2/r2);
	  }
	  else{
	  	  r1 = E->sx[m][3][jj]*E->sphere.dradius;
		  r2 = rr*E->sphere.dradius;
		  g2 = g1*r1*r1/r2/r2 + 4./3.*con*rho2*(r2-pow(r1,3.)/r2/r2);
	      	  r2 = E->sx[m][3][jj+1]*E->sphere.dradius;
	      	  g1 = g1*r1*r1/r2/r2 + 4./3.*con*rho2*(r2-pow(r1,3.)/r2/r2);
      ...
      for (j=1;j<=E->lmesh.elx;j++)  {
         ...
         E->elambda[lev][m][el] = tmp2/E->ve_data_cont.shear_mod;
         E->esmu[lev][m][el] = tmp3/E->ve_data_cont.shear_mod;
         E->egrav[lev][m][el] = tmp4/E->data.grav_acc;
         E->Maxwelltime[m][el] = E->esmu[lev][m][el];
         for(k=1;k<=vpoints[E->mesh.nsd];k++)
            EEta[m][ (el-1)*vpts+k ] = tmp5/E->data.ref_viscosity;
         }	  
	...
	/*	
		if(E->sphere.capid[1] == 1 && E->parallel.me_loc[1]==0 && E->parallel.me_loc[2]==0){
			...
			for(element=1; element<=E->lmesh.elz; element++){
				//if((element-1)%(E->lmesh.nox*E->lmesh.noy)==0){

				fprintf(E->fp, "%e %e %e %e %e %e", E->ECO[E->mesh.levmax][m][element].centre[3],
											E->erho[E->mesh.levmax][m][element],
											E->elambda[E->mesh.levmax][m][element],
											E->esmu[E->mesh.levmax][m][element],
											E->egrav[E->mesh.levmax][m][element],
											E->EVI[E->mesh.levmax][m][element*8]);
				fprintf(E->fp, "\n");
				//}
			}
		}
	...
	*/	
		...
		/*
		if(E->parallel.me==0){
		    printf("----- debug E->ID (in get_system_viscosity; after ele_to_nodes for grav) ------\n");
		int m, i_debug, node_debug, eqn_debug,level_debug;
		int ends=8;
		for(m=1;m<=E->sphere.caps_per_proc;m++)
		for(i_debug=1;i_debug<=ends;i_debug++){
...
//MPI_Barrier(MPI_COMM_WORLD);	
	...
	for(m=1;m<=E->sphere.caps_per_proc;m++)
	  for(i=1;i<=E->lmesh.ELZ[lev];i++) {
		...
		/*
		  for (k=1;k<=vpts;k++){
			g1 = E->egrav[lev][m][(i-2)*vpts+k]*E->data.grav_acc;
			r1 = tmp[k];
			r2 = 1./rtf[3][k]*E->sphere.dradius;
			tmp[k] = r2;
			rho2 = E->erho[lev][m][(i-1)*vpts+k]*E->data.density;
			g2 = g1*r1*r1/r2/r2 + 4./3.*con*rho2*(r2-pow(r1,3.)/r2/r2);
			E->egrav[lev][m][(i-1)*vpts+k] = g2/E->data.grav_acc;
		  }
		  */
		}
...
void visc_from_file(E,EEta,visc)
     struct All_variables *E;
     float **EEta,**visc;
{
...
//if(E->parallel.me==0) fprintf(stderr,"%d %d %d rr= %g %g %g\n",jj,i1,i2,rr,temp1,temp2);
      ...
      for (j=1;j<=E->lmesh.elx;j++)  {
         ...
         el = jj + (j-1)*E->lmesh.elz + (i-1)*E->lmesh.elz*E->lmesh.elx;
         for(k=1;k<=vpoints[E->mesh.nsd];k++)
            // EEta[m][ (el-1)*vpts+k ] = pow(10,temp1)/E->data.ref_viscosity;  // use power if log10 used above
            EEta[m][ (el-1)*vpts+k ] = 1/temp1/E->data.ref_viscosity;  // use Harmonic Mean
         }
  ...
  visc_from_gint_to_nodes(E,EEta,visc,E->mesh.levmax);

  return_horiz_ave_f(E,visc,ave_visc);

/*
  for(m=1;m<=E->sphere.caps_per_proc;m++) {
    for(k=1;k<=E->lmesh.noz;k++)   {
      sprintf(output_file,"visc.%d.%d",k,E->parallel.me);
      fp1 = fopen(output_file,"w");

      fprintf(fp1,"%.6e %.6e\n",E->sx[m][3][k],6370.0*(1.0-E->sx[m][3][k]));
      for(i=1;i<=E->lmesh.noy;i++)  
        for(j=1;j<=E->lmesh.nox;j++)  {
          nn=k+(j-1)*E->lmesh.noz+(i-1)*E->lmesh.noz*E->lmesh.nox;
          visc[m][nn] = visc[m][nn]*E->data.ref_viscosity;
          fprintf(fp1,"%.5e %.5e %.5e\n",90-180*E->sx[m][1][nn]/M_PI,180*E->sx[m][2][nn]/M_PI,visc[m][nn]);
          visc[m][nn] = visc[m][nn]/E->data.ref_viscosity;
          }
...
*/
void model_from_file_3D(E,element_var, filename, normalization)
     struct All_variables *E;
     float **element_var; // 
     char * filename;
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++)
    for (jj=1;jj<=E->lmesh.elz;jj++)  {
      ...
      for (j=2;j<=nnfi-1;j++)  {
        ...
        fgets(input_s,200,fp1);
        sscanf(input_s,"%g",&temp1);
        visc_reg[k] = temp1;       // another possibility is visc_reg[k] = log10f(temp1);
        }
...
//if(E->parallel.me==0) fprintf(stderr,"%d %d %d rr= %g %g %g\n",jj,i1,i2,rr,temp1,temp2);
        ...
        for (j=2;j<=nnfi-1;j++)  {
          ...
          fgets(input_s,200,fp1);
          sscanf(input_s,"%g",&temp3);
          visc_reg[k] = visc_reg[k]*temp1 + temp3*temp2;  // also use log10f here if used above
          }
        }
      fclose(fp1);
      ...
      for (j=1;j<=E->lmesh.elx;j++)  {
         ...
         el = jj + (j-1)*E->lmesh.elz + (i-1)*E->lmesh.elz*E->lmesh.elx;
         for(k=1;k<=vpoints[E->mesh.nsd];k++)
            element_var[m][el] = temp1 / normalization;  // use power if log10 used above
         }
 ...
 // ele_to_nodes(E, element_var, node_var, lev);

 // return_horiz_ave_f(E,node_var,ave_visc);
...
//   if (E->parallel.me==0)  {
//     fprintf(E->fp_out,"average value vs radius after the interpolation to FE grid\n"); 
//     for(k=1;k<=E->lmesh.noz;k++)   {
//       fprintf(E->fp_out,"%d %.5e %.5e\n",k,E->sx[1][3][k],ave_visc[k]*normalization); 
//       }
//     }
 ...
 */      }
	  ...
	  Sxyz6 = VV[k][i]*
        ...
        a = E->Maxwelltime[m][e]/(2.0*visc1)*E->advection.timestep;
        visc0 = E->Maxwelltime[m][e]/(1.0+a);
