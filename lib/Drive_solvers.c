...
if (  !E->viscosity.SDEPV  )   {
    ...
    if(need_visc_update(E)){
      ...
      construct_stiffness_B_matrix(E);
		...
		/*
		if(E->parallel.me==0){
			printf("---debug E->BI , neq=%d -----\n",neq);
			int debug_lv;
			for(debug_lv=E->mesh.levmax;debug_lv>=E->mesh.levmin;debug_lv--)
			{
				char debug_outfile[255];
				sprintf(debug_outfile, "debug_BI.%d.%d",E->monitor.solution_cycles,debug_lv);
				FILE * debug_fp;
				debug_fp = fopen(debug_outfile, "w");
				int debug_i;
				for(debug_i=0; debug_i<E->lmesh.NEQ[debug_lv]; debug_i++){

					if(debug_i%3==0) fprintf(debug_fp,"\n");
					fprintf(debug_fp,"%e  ", E->BI[debug_lv][1][debug_i]);
				}
				fclose(debug_fp);
			}
		}
		...
		if(E->parallel.me==0){
			printf("---debug E->TWW -------\n");
				char debug_outfile[255];
				sprintf(debug_outfile, "debug_TWW_before_ass_force.%d",E->monitor.solution_cycles);
				FILE * debug_fp;
				debug_fp = fopen(debug_outfile, "w");
				int debug_e, debug_i;
    ...
    do {
	  E->monitor.count=count; // add by tao;
     		if(E->parallel.me==0){
			time = CPU_time0();
			fprintf(E->fp,"DEBUG: before assemble_force, time = %f \n",time-time0);
		} 
	  assemble_forces(E,count);
		
		//if(E->parallel.me==0)
  		/*{
			printf("---debug E->F (count = %d), neq=%d -----\n", count,neq);
			char debug_outfile[255];
			sprintf(debug_outfile, "debug_F.%d.%d.proc%d",E->monitor.solution_cycles,count,E->parallel.me);
			FILE * debug_fp;
			debug_fp = fopen(debug_outfile, "w");
			int debug_i;
			for(debug_i=0; debug_i<neq; debug_i++){
				...
				if(debug_i%3==0) fprintf(debug_fp,"\n");
				fprintf(debug_fp,"%e  ", E->F[1][debug_i]);
			}
			fclose(debug_fp);
		}
		*/
                if(E->parallel.me==0){
                        ...
                        fprintf(E->fp,"DEBUG: after assemble_force, time = %f \n",time);
                } 
      ...
      solve_constrained_flow_iterative(E);
                ...
                if(E->parallel.me==0){
                        time = CPU_time0() - time0;
                        fprintf(E->fp,"DEBUG: after solve_constrained_flow_iterative, time = %f \n",time);
                }
		/*
		if(E->parallel.me==0){
			printf("---debug Earth model ----\n");
			int debug_lv;
			for(debug_lv=E->mesh.levmax;debug_lv>=E->mesh.levmin;debug_lv--)
			{
				char debug_outfile[255];
				sprintf(debug_outfile, "debug_EarthModel.%d.%d.%d",E->monitor.solution_cycles,count,debug_lv);
				FILE * debug_fp;
				debug_fp = fopen(debug_outfile, "w");
				int debug_i;
				for(debug_i=1; debug_i<=E->lmesh.NEL[debug_lv]; debug_i++)
				{
						fprintf(debug_fp, "%e %e %e %e %e %e\n", E->ECO[debug_lv][1][debug_i].centre[3],
												E->erho[debug_lv][1][debug_i],
												E->elambda[debug_lv][1][debug_i],
												E->esmu[debug_lv][1][debug_i],
												E->egrav[debug_lv][1][debug_i],
												E->EVI[debug_lv][1][debug_i*8]);
				}
				fclose(debug_fp);
			}
	...
	//	if(E->parallel.me==0)
	/*	{
			printf("---debug E->U (count = %d), neq=%d -----\n", count,neq);
			char debug_outfile[255];
			sprintf(debug_outfile, "debug_U.%d.%d.proc%d",E->monitor.solution_cycles,count,E->parallel.me);
			FILE * debug_fp;
			debug_fp = fopen(debug_outfile, "w");
			int debug_i;
			for(debug_i=0; debug_i<neq; debug_i++){
...
//  } while ( (count<50) && ((dUdot_mag > E->viscosity.sdepv_misfit) || (dPW_mag > /*3* */ E->viscosity.sdepv_misfit)) && E->ve_data_cont.SELFG);
...
//   do {
	...
	for (i=0;i<neq;i++) {
	  ...
	  oldU[m][i] = E->U[m][i];
       ...
       if(E->parallel.me==0) {
 	 fprintf(stderr,"dU %.4e (%.4e) %.4e dPW %.4e (%.4e) for iteration %d\n",
		dUdot_mag,Udot_mag,dUdot_mag1,dPW_mag,PW_mag, count);
         time=CPU_time0()-time0;
	 fprintf(E->fp,"!!! iteration=%d relative change %g %g %g m0 m1 %g %g timestep %d elapse time %g\n",
		count,dUdot_mag,dUdot_mag1,dPW_mag,pwold[0],pwold[1],E->monitor.solution_cycles,time);
       fflush(E->fp);
       }
