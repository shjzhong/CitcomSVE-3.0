...
void exchange_sphcs(E, sphc0, sphs0, sphc1, sphs1)
 struct All_variables *E;
 double *sphc0, *sphs0, *sphc1, *sphs1;
 {
 ...
 if (been_here ==0 )   {
  ...
  if (E->parallel.me_loc[3] == 0)  {
    for (p=0;p<E->sphere.hindice;p++)  {
      SV[p                  ] = sphc1[p];
      SV[p+E->sphere.hindice] = sphs1[p];
      }
    target_proc= E->parallel.me + E->parallel.nprocz-1;
    }
  else if (E->parallel.me_loc[3] == E->parallel.nprocz-1)  {
    for (p=0;p<E->sphere.hindice;p++)  {
      SV[p                  ] = sphc0[p];
      SV[p+E->sphere.hindice] = sphs0[p];
      }
    target_proc = E->parallel.me - E->parallel.nprocz+1;
    }
  else
  ...
  MPI_Sendrecv(SV,idb,MPI_DOUBLE,target_proc,1,
               RV,idb,MPI_DOUBLE,target_proc,1,MPI_COMM_WORLD,&status1);


  if (E->parallel.me_loc[3] == 0)  {
    for (p=0;p<E->sphere.hindice;p++)  {
      sphc0[p] = RV[p                  ];
      sphs0[p] = RV[p+E->sphere.hindice];
      }
    }
  else if (E->parallel.me_loc[3] == E->parallel.nprocz-1)  {
    for (p=0;p<E->sphere.hindice;p++)  {
      sphc1[p] = RV[p                  ];
      sphs1[p] = RV[p+E->sphere.hindice];
      }
    }
