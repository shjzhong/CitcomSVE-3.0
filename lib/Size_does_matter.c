...
//
  ...
  for (m=1;m<=E->sphere.caps_per_proc;m++)   {
    for(e=1;e<=E->lmesh.snel;e++)          {
      get_global_1d_shape_fn_3(E,e,0,lev,m);
      get_global_1d_shape_fn_3(E,e,1,lev,m);
      }
    }
...
/* version */
/* $Id: Size_does_matter.c 14639 2009-04-08 23:31:11Z tan2 $ */
...
/*   ======================================================================
     ======================================================================  */
void get_global_1d_shape_fn_3(E,ee,ic,lev,m)
     struct All_variables *E;
     int ic,ee,m,lev;
{
  int ii,i,k,d,el,node,e;
 
  double jacobian,dx1,dx2,dx3,centre[4];
  double determinant();
  double cofactor();
  ...
  el = (ic==1)?(ee*E->lmesh.ELZ[lev]):((ee-1)*E->lmesh.ELZ[lev]+1);

  to = E->ECO[lev][m][el].centre[1];
  fo = E->ECO[lev][m][el].centre[2];
...
//fprintf(stderr,"aaa %d %d %g %g %g\n",E->parallel.me,el,to,fo,dxdy[3][3]);

  ii = 0;
  for(i=1;i<=oned;i++) {     /* nodes */
     e = i+ic*oned;
     node = E->IEN[lev][m][el].node[e];
     xx[1][i] = E->X[lev][m][1][node]*dxdy[1][1]
              + E->X[lev][m][2][node]*dxdy[1][2]
              + E->X[lev][m][3][node]*dxdy[1][3];
     xx[2][i] = E->X[lev][m][1][node]*dxdy[2][1]
              + E->X[lev][m][2][node]*dxdy[2][2]
              + E->X[lev][m][3][node]*dxdy[2][3];
     xx[3][i] = E->X[lev][m][1][node]*dxdy[3][1]
              + E->X[lev][m][2][node]*dxdy[3][2]
              + E->X[lev][m][3][node]*dxdy[3][3];
     }

  if (ic==0) 
    E->gDA0[m][ee].vpt[oned+1] = 0.0;
  else
    E->gDA1[m][ee].vpt[oned+1] = 0.0;
  ...
  for(k=1;k<=oned;k++)    { /* all of the vpoints*/
     ...
     if (ic==0)   {
       E->gDA0[m][ee].vpt[k] = jacobian;
       E->gDA0[m][ee].vpt[1+oned] += jacobian;
       }
     else if (ic==1)   {
       E->gDA1[m][ee].vpt[k] = jacobian;
       E->gDA1[m][ee].vpt[1+oned] += jacobian;
       }
...
/*   ======================================================================
     ======================================================================  */
void get_global_1d_shape_fn_2(E,el,GM,dGammax,ic,lev,m)
     struct All_variables *E;
     int ic,el,m,lev;
     struct Shape_function1 *GM;
     struct Shape_function1_dA *dGammax;
...
void get_potential_coff(E,lev,m)
struct All_variables *E;
int lev,m;
{
 int n,j,k,k1,ll,mm,p;
 double modified_plgndr_a(),t[5],f[5];

 for (n=1;n<=E->lmesh.nsf;n++)   {

   t[1] = E->SX[lev][m][1][n*E->lmesh.NOZ[lev]];
   f[1] = E->SX[lev][m][2][n*E->lmesh.NOZ[lev]];

   for (mm=0; mm<=E->output.llmax; mm++)      {
     E->Tbl_cs[m][mm][n] = cos(mm*f[1]);
     E->Tbl_sn[m][mm][n] = sin(mm*f[1]);
     }

   for (ll=0;ll<=E->output.llmax;ll++)
     for (mm=0; mm<=ll; mm++)                           {
       p = E->sphere.hindex[ll][mm];
       E->Tbl_lm[m][p][n] = modified_plgndr_a(ll,mm,t[1]);
       }
   }
...
/* ==========================================================
 danger!!! assume a small deformation in t and f directions
 ============================================================ */

void get_sphere_expansion_coff(E,e,lev,m)
struct All_variables *E;
int e,lev,m;
{
 int j,k,k1,ll,mm,p;
 ...
 for(k1=1;k1<=onedvpoints[E->mesh.nsd];k1++)   {
   t[k1] = E->sx[m][1][E->ien[m][e*E->lmesh.elz].node[k1+4]];
   f[k1] = E->sx[m][2][E->ien[m][e*E->lmesh.elz].node[k1+4]];
   }

 for (mm=0; mm<=E->output.llmax; mm++)                 
   for(k=1;k<=onedvpoints[E->mesh.nsd];k++)   {
     E->Sph_Harm_Tblcs[m][mm][e].cs[k] = 0.0;
     E->Sph_Harm_Tblcs[m][mm][e].sn[k] = 0.0;
     for(k1=1;k1<=onedvpoints[E->mesh.nsd];k1++)   {
       E->Sph_Harm_Tblcs[m][mm][e].cs[k]+=cos(mm*f[k1])*E->M.vpt[GMVINDEX(k1,k)];
       E->Sph_Harm_Tblcs[m][mm][e].sn[k]+=sin(mm*f[k1])*E->M.vpt[GMVINDEX(k1,k)];
       }
     }
 ...
 for (ll=0;ll<=E->output.llmax;ll++)
   for (mm=0; mm<=ll; mm++)                           {
     p = E->sphere.hindex[ll][mm];
     for(k=1;k<=onedvpoints[E->mesh.nsd];k++)   {
       E->Sph_Harm_Tbllm[m][p][e].lm[k] = 0.0;
       for(k1=1;k1<=onedvpoints[E->mesh.nsd];k1++)   {
         E->Sph_Harm_Tbllm[m][p][e].lm[k]+= modified_plgndr_a(ll,mm,t[k1])*E->M.vpt[GMVINDEX(k1,k)];
         }
       }
     }
